...
public class PortConnect {
    ...
    private static List<byte[]> resultList = Collections.synchronizedList(new ArrayList<>());
     ...
     * @param data
     */
    private static void addToList(byte[] data) {
        synchronized (resultList) {
            resultList.add(data);
        }
    }
        ...
        public void run() {
            ...
            try {
                while (true) {
                    ...
                    System.arraycopy(buffer, 0, data_arr, 0, len);
                    addToList(data_arr);
