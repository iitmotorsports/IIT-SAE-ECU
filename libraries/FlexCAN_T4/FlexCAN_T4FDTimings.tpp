...
FCTPFD_FUNC bool FCTPFD_OPT::setBaudRate(CANFD_timings_t config, uint8_t nominal_choice, uint8_t flexdata_choice, FLEXCAN_RXTX listen_only, bool advanced) {
                    ...
                    for (double pseg = 1; pseg <= pseg1; pseg++) {
                        if (req_smp != 0) {
                            ...
                            if (pseg_2 >= ipt && pseg_2 <= pseg2 && pseg_1 > 0 && pseg_1 <= pseg1) {
                                ...
                                for (uint32_t i = 0; i < (scale * 149 / 1200) + 1; i++)
                                    _sync[i] = 'a';
    ...
    if (nominal_choice && flexdata_choice) {
        ...
        if (fdcbt_setting) { // SET BOTH CBT AND FDCBT HERE
            ...
            if (advanced)
                Serial.println(FLEXCANb_CTRL1(_bus), HEX);
            if (advanced)
                Serial.print("CBT: 0x");
            if (advanced)
                Serial.println(FLEXCANb_CBT(_bus), HEX);
            if (advanced)
                Serial.print("FDCBT: 0x");
            if (advanced)
                Serial.println(FLEXCANb_FDCBT(_bus), HEX);
            if (advanced)
                Serial.print("FDCTRL: 0x");
            if (advanced)
                Serial.println(FLEXCANb_FDCTRL(_bus), HEX);
            FLEXCAN_ExitFreezeMode();
            return 1;
        } else
            return 0;
    } else
        setBaudRateFD(config, 0, advanced); /* continue list printout for FD */
    while (Serial.read() != 'c')
        ;
    return 1;
}
